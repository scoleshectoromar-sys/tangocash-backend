...
class WSFEv1(BaseWS):
    ...
    def CrearFactura(self, concepto=1, tipo_doc=80, nro_doc="", tipo_cbte=1, punto_vta=0,
        ...
        # inicializo la lista de facturas (requerimiento múltiple a.k.a. lote)
        if self.factura is None:
            self.facturas = []
        self.facturas.append(fact)

        # apunto la factura actual para los otros metodos agregar:
        self.factura = fact
        return True
    ...
    def CAESolicitar(self):
        # genero los registros multiples de factura
        regs = []
        for f in self.facturas:
            regs.append({'FECAEDetRequest': {
                    'Concepto': f['concepto'],
                    'DocTipo': f['tipo_doc'],
                    'DocNro': f['nro_doc'],
        ...
        # invóco al método remoto:
        ret = self.client.FECAESolicitar(
            Auth={'Token': self.Token, 'Sign': self.Sign, 'Cuit': self.Cuit},
            FeCAEReq={
                'FeCabReq': {'CantReg': len(regs), 
                    'PtoVta': f['punto_vta'], 
        ...
        if 'FeCabResp' in result:
            ...
            self.repuestas = result['FeDetResp']
            self.LeerRespuesta(0)
        self.__analizar_errores(result)
        
        # limpio la factura (internamente)
        self.factura = self.facturas = None
        return self.CAE

    def LeerRespuesta(self, n):
        fedetresp = self.respuestas[n]['FECAEDetResponse']
        
        # Reprocesar en caso de error (recuperar CAE emitido anteriormente)
        if n == 0 and self.Reprocesar and ('Errors' in result or 'Observaciones' in fedetresp):
            # por el momento solo la primer factura:
            f = self.factura
...
def main():
    ...
    if "--prueba" in sys.argv:
        ...
        import time
        t0 = time.time()
        if not '--lote' in sys.argv:
            wsfev1.CAESolicitar()
        else:
            wsfev1.CAESolicitarLote()
            print "Tamaño request", len(wsfev1.XmlRequest)
            print "Tamaño response", len(wsfev1.XmlResponse)
        t1 = time.time()
        print "Tiempo", t1 - t0
